...
internal class DocumentsGenerator
{
    ...
    public bool GenerateDocumentsFromTemplates()
    {
        try
        {
            ...
            var excludedExtensions = new HashSet<string>(StringComparer.CurrentCultureIgnoreCase)
            {
                ".cs",
                ".bat"
            };
            ...
            IterateFolderFiles(
                demoDocsFolder, SrcDocFilesPathRelativeToSolutionFolder, filesData =>
                {
                    ...
                    if (excludedExtensions.Contains(Path.GetExtension(filesData.srcFilePathRelativeToSolutionFolder)))
                        return;
